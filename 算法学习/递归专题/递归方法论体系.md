...
| 类型 | 集合来源 | check数组 | index参数 | for起点 | 递归参数 | 典型题目 |
|------|---------|-----------|-----------|---------|----------|----------|
| 全排列 | 同一个数组 | ✅ 需要 | ❌ 不需要 | 0 | 不变 | LeetCode 46/47 |
| 子集/组合 | 同一个数组 | ❌ 不需要 | ✅ 需要 | index | i+1 | LeetCode 78/77 |
| 多集合组合 | 不同集合 | ❌ 不需要 | ✅ 需要 | 0 | index+1 | LeetCode 17 |
| 剪枝生成 | 同类元素 | ❌ 不需要 | 特殊变量 | - | 状态参数 | LeetCode 22 |
